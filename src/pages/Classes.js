...
export default function Classes() {
  ...
  // Combine date and time into MySQL-friendly format (no timezone shift)
                  ...
                  editingId === c.id ? (
                    <tr key={c.id} style={{ backgroundColor: '#f9f9f9' }}>
                      ...
                      <td style={tdStyle}>
                        ...
                        <input name="start_time" type="time" value={editForm.start_time} onChange={handleEditChange} style={inputStyle} />
                      </td>
                      <td style={tdStyle}>
                        <input name="end_date" type="date" value={editForm.end_date} onChange={handleEditChange} style={inputStyle} />
                        <input name="end_time" type="time" value={editForm.end_time} onChange={handleEditChange} style={inputStyle} />
                      </td>
                      <td style={tdStyle}>
                        <button onClick={saveEdit} style={saveBtnStyle}>Save</button>{' '}
                        <button onClick={cancelEditing} style={cancelBtnStyle}>Cancel</button>{' '}
                        <button onClick={() => deleteClass(c.id)} style={deleteBtnStyle}>Delete</button>
                      </td>
                    </tr>
                  ) : (
                    <tr key={c.id}>
                      ...
                      <td style={tdStyle}>
  {c.start_time ? new Date(c.start_time).toLocaleString('en-US', {
    dateStyle: 'short',
    timeStyle: 'short',
    hour12: true,
...
<td style={tdStyle}>
  {c.end_time ? new Date(c.end_time).toLocaleString('en-US', {
    dateStyle: 'short',
    timeStyle: 'short',
    hour12: true,
    timeZone: 'America/Chicago',
...
const inputStyle = { width: '100%', padding: '6px 8px', fontSize: '14px', borderRadius: 4, border: '1px solid #ccc', marginBottom: 4 };
...
const deleteBtnStyle = { backgroundColor: '#9c27b0', color: 'white', border: 'none', padding: '8px 16px', cursor: 'pointer', borderRadius: 6, fontWeight: 'bold' };
...
const formRowStyle = { marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12 };
