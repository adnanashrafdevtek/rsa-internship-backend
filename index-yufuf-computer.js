...
// ✅ ================= AGENT JWT ROUTES (ADDED) =================
// Mint a token for Langflow agent (protect this!)
//
// Call with header:
// x-agent-mint-key: <AGENT_MINT_KEY from .env>
app.post("/api/agent/token", (req, res) => {
  try {
    ...
    // Put ONLY what you need in the token
    const payload = {
      type: "agent",
      role: "langflow",
      scope: ["calendar:read", "calendar:write"], // adjust permissions as needed
    };

    const token = signAgentToken(payload);
    res.json({ token });
  } catch (err) {
    console.error("Error minting agent token:", err);
    res.status(500).json({ error: "Failed to mint token" });
  }
});

// Test route to verify agent token works
app.post("/api/agent/ping", requireAuth, requireAgent, (req, res) => {
  res.json({
    ok: true,
...
app.delete('/api/classes/:classId/students/:studentId', async (req, res) => {
  ...
  try {
    ...
    res.json({ message: 'Student removed from this class' });
  } catch (err) {
